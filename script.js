...
function renderPusatLaporanMenu() {
  ...
  document.getElementById("content").innerHTML = `
    <div class="page-wrap" style="width: 100%; max-width: 480px; margin: 0 auto; padding: 12px; box-sizing: border-box;">
      <div class="bin-header" style="margin-bottom: 12px;">
        <div class="bin-title" style="font-size: 16px; font-weight: bold; color: #fff;"><i class="fa-solid fa-folder-open"></i> PUSAT LAPORAN & RIWAYAT</div>
        <div class="bin-subtitle" style="font-size: 11px; opacity: 0.6;">Living Plaza Balikpapan — Kontrol Data Logistik</div>
      </div>

      <div style="display: flex !important; flex-direction: row !important; flex-wrap: nowrap !important; gap: 4px; overflow-x: auto !important; padding-bottom: 8px; margin-bottom: 12px; -webkit-overflow-scrolling: touch; width: 100%;">
        <button id="tabLpBincard" class="active" onclick="switchSubMenuLaporan('bincard')" style="flex: 0 0 auto !important; width: auto !important; min-width: 75px; padding: 6px 10px; background: #1e293b; border: 1px solid #334155; color: #fff; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; white-space: nowrap;">Bin Card</button>
        <button id="tabLpDatang" onclick="switchSubMenuLaporan('datang')" style="flex: 0 0 auto !important; width: auto !important; min-width: 75px; padding: 6px 10px; background: #1e293b; border: 1px solid #334155; color: #fff; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; white-space: nowrap;">barang Masuk</button>
        <button id="tabLpMusnah" onclick="switchSubMenuLaporan('musnah')" style="flex: 0 0 auto !important; width: auto !important; min-width: 75px; padding: 6px 10px; background: #1e293b; border: 1px solid #334155; color: #fff; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; white-space: nowrap;">Pemusnahan barang</button>
        <button id="tabLpOpname" onclick="switchSubMenuLaporan('opname')" style="flex: 0 0 auto !important; width: auto !important; min-width: 75px; padding: 6px 10px; background: #1e293b; border: 1px solid #334155; color: #fff; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; white-space: nowrap;">stock Opname</button>
        <button id="tabLpMatriks" onclick="switchSubMenuLaporan('matriks')" style="flex: 0 0 auto !important; width: auto !important; min-width: 75px; padding: 6px 10px; background: #1e293b; border: 1px solid #334155; color: #fbbf24; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; white-space: nowrap;"><i class="fa-solid fa-table"></i> Laporan inventory</button>
      </div>
...
function switchSubMenuLaporan(pilihan) {
  ...
  if (pilihan === 'bincard') {
    ...
    api({ action: "getRiwayatOpnameMurni" }).then(res => {
      ...
      document.getElementById("fLpOpnameBulan").value = getCurrentMonth(); 
      filterEngineOpname();
    });
...
// ================= MESIN PENYARING DATA (SOPAN TERHADAP FILTER KOSONG) =================
function filterEngineBincard() {
  let tglVal = document.getElementById("fLpBincardTgl").value;
  let filtered = cachePusatLaporan.bincard;
  
  // Perbaikan: Jika filter tanggal diisi, saring. Jika kosong, biarkan semua data mengalir keluar.
  if (tglVal) {
    filtered = cachePusatLaporan.bincard.filter(d => getTglKey(d.tanggal) === tglVal);
  }
...
function filterEngineDatang() {
  ...
  if (tglVal) {
    filtered = cachePusatLaporan.datang.filter(d => d.tanggalRaw && d.tanggalRaw.substring(0,10) === tglVal);
  }
...
function filterEngineMusnah() {
  ...
  if (tglVal) {
    filtered = cachePusatLaporan.musnah.filter(d => d.tanggalRaw && d.tanggalRaw.substring(0,10) === tglVal);
  }
...
function filterEngineOpname() {
  ...
  if (blnVal) {
    filtered = cachePusatLaporan.opname.filter(d => d.tanggalRaw && d.tanggalRaw.substring(0,7) === blnVal);
  }
